...
EXTEND_TASK_VARIANTS = (
    ...
    "в списке variants — добавь их с написаниями, которыми их набирают. Признаки, города, под-группы, этапы и "
    "особенности не добавляй — ими заняты другие части; groups оставь пустым."
)
EXTEND_TASK_AXES = (
...
def extend_prompts(ctx: dict, cur: dict, keys_index: dict[str, int], chunks: int) -> list[str]:
    ...
    иначе часть 0 — варианты (AXES_MODEL), часть 1 — оси + недостающие пункты, части 2..N — доли чек-листа."""
    mp = map_for_prompt(cur, keys_index)
    checklist = list(cur["journey"]) + list(cur["specifics"])
    n_parts = max(1, chunks - 2)

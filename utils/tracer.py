...
Отслеживает путь каждого ключа через ВСЕ фильтры:
  ...
  7. l3_filter            (DeepSeek LLM: финальная классификация GREY)
...
Использование:
  ...
  # Для L3 (DeepSeek LLM):
  tracer.after_l3_filter(valid, trash, error, l3_stats, l3_trace)
  # l3_trace: [{keyword, label, tail}]
...
class FilterTracer:
    ...
    def after_l3_filter(self, valid: List[Any], trash: List[Any], error: List[Any],
                        l3_stats: Dict = None, l3_trace: List[Dict] = None):
        """
        Специальный метод для L3 — DeepSeek LLM классификатор.
        Три исхода: VALID / TRASH / ERROR.
        
        Args:
            valid: ключи которые DeepSeek пометил VALID
            trash: ключи которые DeepSeek пометил TRASH
            error: ключи которые не удалось обработать (остаются в grey)
            l3_stats: статистика L3 (input_grey, l3_valid, l3_trash, api_time...)
            l3_trace: [{keyword, label, tail, l2_info}]
        """
        if not self.enabled:
            return
        
        filter_name = "l3_filter"
        if filter_name not in self.stages:
            logger.warning(f"[TRACER] after_l3_filter без before_filter!")
            return
        
        stage = self.stages[filter_name]
        valid_set = self._extract_keywords(valid)
        trash_set = self._extract_keywords(trash)
        error_set = self._extract_keywords(error)
        
        stage["after"] = valid_set | error_set  # "прошедшие" = VALID + ERROR
        stage["blocked"] = trash_set            # "заблокированные" = TRASH
        stage["grey"] = error_set               # ERROR → grey (fallback)
        stage["valid"] = valid_set              # VALID
        stage["time"] = time.time() - stage.get("_start", time.time())
        stage["l3_stats"] = l3_stats or {}
        
        # Сохраняем детальный трейс L3
        if l3_trace:
            stage["l3_details"] = {}
            for rec in l3_trace:
                kw = rec.get("keyword", "").lower().strip()
                ...
                stage["l3_details"][kw] = {
                    "label": label,
                    "tail": rec.get("tail", ""),
                    "source": (l3_stats or {}).get("model", "deepseek"),
                }
                
                stage["reasons"][kw] = f"[{label}] DeepSeek"
        
        # Записываем в keyword_map для L3 TRASH
        for kw in trash_set:
            if kw not in self.keyword_map:
                reason = stage.get("reasons", {}).get(kw, "L3_TRASH")
                self.keyword_map[kw] = {
                    "blocked_by": "l3_filter",
                    "reason": reason,
                }
        
        logger.info(
            f"[TRACER] ◆ l3_filter | VALID: {len(valid_set)} | "
            f"TRASH: {len(trash_set)} | ERROR: {len(error_set)} | "
            f"time: {stage['time']:.3f}s"
        )
        
        if l3_stats:
            logger.info(
                f"[TRACER]   L3 stats: input_grey={l3_stats.get('input_grey', 0)} "
                f"→ l3_valid={l3_stats.get('l3_valid', 0)} "
                f"l3_trash={l3_stats.get('l3_trash', 0)} "
                f"l3_error={l3_stats.get('l3_error', 0)} "
                f"api_time={l3_stats.get('api_time', 0)}s"
            )
    ...
    def finish_request(self) -> Dict:
        ...
        l3_stage = self.stages.get("l3_filter", {})
        ...
        for kw in surviving:
            ...
            # L3 детали
            l3_details = l3_stage.get("l3_details", {}).get(kw)
            if l3_details:
                kw_trace["l3"] = l3_details
            
            valid_trace[kw] = kw_trace
        
        report["valid_keywords"] = valid_trace
        
        # Трейс для GREY ключей (финальные GREY после L3→L2→L0)
        grey_trace = {}
        
        # Берём GREY из последнего активного слоя: L3 > L2 > L0
        if l3_stage:
            final_grey = l3_stage.get("grey", set())
        elif l2_stage:
            final_grey = l2_stage.get("grey", set())
        else:
            final_grey = l0_stage.get("grey", set())
        ...
        for kw in sorted(final_grey):
            ...
            l3_details = l3_stage.get("l3_details", {}).get(kw)
            if l3_details:
                kw_details["l3"] = l3_details
            
            if kw_details:
                grey_trace[kw] = kw_details
            else:
                grey_trace[kw] = {"label": "GREY", "reason": "remaining after all layers"}
        ...
        # L3 статистика
        if l3_stage:
            report["l3_stats"] = l3_stage.get("l3_stats", {})
    ...
    def _extract_keywords(items: List[Any]) -> set:
        ...
        for item in items:
            if isinstance(item, str):
                ...
                q = item.get("query", "") or item.get("keyword", "")
                if q:
    ...
    def get_keyword_trace(self, keyword: str) -> Dict:
        ...
        # Определяем статус с учётом L0/L2/L3 grey
        if blocked_info:
            status = "blocked"
        elif l3_stage and kw in l3_stage.get("grey", set()):
            status = "grey"  # финальный GREY после L3 (ERROR)
        elif l2_stage and kw in l2_stage.get("grey", set()) and not l3_stage:
            status = "grey"  # GREY после L2, L3 не запускался
    ...
    def format_report_text(self) -> str:
        ...
        if grey_kws:
            lines.append(f"")
            lines.append(f"── Grey keywords (remaining after all layers) ({len(grey_kws)}) ──")
        ...
        # L3 статистика
        l3_stats = report.get("l3_stats", {})
        if l3_stats and not l3_stats.get("error"):
            lines.append(f"")
            lines.append(f"── L3 DeepSeek Classifier ──")
            lines.append(
                f"  Input GREY: {l3_stats.get('input_grey', 0)} | "
                f"→ VALID: {l3_stats.get('l3_valid', 0)} | "
                f"TRASH: {l3_stats.get('l3_trash', 0)} | "
                f"ERROR: {l3_stats.get('l3_error', 0)}"
            )
            lines.append(
                f"  API time: {l3_stats.get('api_time', 0)}s | "
                f"Model: {l3_stats.get('model', '?')} | "
                f"Batches: {l3_stats.get('batches', 0)}"
            )
        elif l3_stats and l3_stats.get("error"):
            lines.append(f"")
            lines.append(f"── L3 DeepSeek Classifier ──")
            lines.append(f"  ⚠️ Skipped: {l3_stats.get('error')}")
        ...
        if valid_kws:
            ...
            for kw, info in list(valid_kws.items())[:30]:
                ...
                # L3 tag если ключ пришёл из L3
                l3_tag = ""
                if l3 and l3.get("label") == "VALID":
                    l3_tag = " | L3: ✓"
                ...
                if l0 and l2 and "pmi" in l2:
                    ...
                    lines.append(
                        f"  ✓ '{kw}' | tail='{tail}' | L0: {sigs} | "
                        f"L2: pmi={pmi_val:.2f} cdist={cdist:.3f} → {decision}{l3_tag}"
                    )
                    ...
                    comb = l2.get("combined_score", 0)
                    direct = l2.get("direct_score", 0)
                    lines.append(f"  ✓ '{kw}' | tail='{tail}' | L0: {sigs} | L2: comb={comb:.3f} direct={direct:.3f}{l3_tag}")
                elif l0:
                    # Только L0 (VALID на L0, не пошёл в L2)
                    tail = l0.get("tail", "")
                    sigs = ", ".join(l0.get("signals", [])) or "—"
                    decided = l0.get("decided_by", "")
                    lines.append(f"  ✓ '{kw}' | tail='{tail}' | L0: {sigs} | by: {decided}{l3_tag}")
                elif l2 and "pmi" in l2:
                    # Только tri-signal L2
                    pmi_val = l2.get("pmi", 0)
                    cdist = l2.get("centroid_dist", 0)
                    decision = l2.get("decision", "")
                    lines.append(f"  ✓ '{kw}' | L2: pmi={pmi_val:.2f} cdist={cdist:.3f} → {decision}{l3_tag}")
                elif l2:
                    comb = l2.get("combined_score", 0)
                    direct = l2.get("direct_score", 0)
                    lines.append(f"  ✓ '{kw}' | L2: comb={comb:.3f} direct={direct:.3f}{l3_tag}")
                elif l3:
                    lines.append(f"  ✓ '{kw}' | L3: {l3.get('source', 'deepseek')} → VALID")
                else:

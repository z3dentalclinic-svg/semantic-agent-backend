...
# ms_0.6: рамка — выбранные ключи ОПИСЫВАЮТ, что продаёт рекламодатель, а не исчерпывают его запросы; минус — только
#         несовместимый интент; отсутствие слова в выборе доказательством не считается; чужой бренд — явное исключение;
#         гео задаёт таргетинг. На «грузоперевозки 10 тонн» с выбором «одесса / одесса цена» прежняя рамка резала
#         стоимость/расценки/км/область/песка/фура/щебенки/яблок — всё клиенты (Andrew).
# ms_0.5 (откат) первые строки: "Рекламодатель показывает объявления только по этим запросам:\n{selected}\n\n"

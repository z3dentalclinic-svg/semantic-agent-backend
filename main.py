...
class KeywordParser:
    ...
    def get_morphological_forms(self, word: str, language: str) -> List[str]:
        ...
        - RU, UK: pymorphy2 (все формы из лексемы)
        ...
        print(f"🔍 DEBUG: Получение форм для '{word}', язык: {language}")
        
        if language.lower() in ['ru', 'uk']:
            # Русский и Украинский - pymorphy2
            print(f"🔍 DEBUG: Пытаемся импортировать pymorphy2...")
            try:
                import pymorphy2
                print(f"✅ DEBUG: pymorphy2 импортирован успешно!")
                print(f"🔍 DEBUG: Версия: {pymorphy2.__version__ if hasattr(pymorphy2, '__version__') else 'unknown'}")
                
                # Создаём анализатор
                print(f"🔍 DEBUG: Создаём MorphAnalyzer...")
                morph = pymorphy2.MorphAnalyzer()
                print(f"✅ DEBUG: MorphAnalyzer создан!")
                
                # Парсим слово
                print(f"🔍 DEBUG: Парсим слово '{word}'...")
                parsed = morph.parse(word)
                print(f"✅ DEBUG: Получено {len(parsed)} вариантов разбора")
                
                if parsed:
                    # Берем первый вариант разбора
                    p = parsed[0]
                    print(f"🔍 DEBUG: Первый вариант: {p.tag}")
                    
                    # Получаем все формы из лексемы
                    print(f"🔍 DEBUG: Извлекаем формы из лексемы...")
                    for form in p.lexeme:
                        forms.add(form.word)
                    
                    print(f"✅ DEBUG: Извлечено {len(forms)} форм")
                else:
                    print(f"⚠️ DEBUG: parsed пустой!")
                
                print(f"📖 Морфология (RU/UK): '{word}' → {len(forms)} форм через pymorphy2")
                
            except ImportError as e:
                print(f"❌ DEBUG: ImportError при импорте pymorphy2: {e}")
                print(f"⚠️ pymorphy2 не установлен. Используем только базовую форму.")
                print(f"   Установите: pip install pymorphy2")
            except Exception as e:
                print(f"❌ DEBUG: Неожиданная ошибка при работе с pymorphy2: {e}")

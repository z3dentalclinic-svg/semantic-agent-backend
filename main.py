...
# ============================================
# GEO BLACKLIST ДЛЯ ПРЕ-ФИЛЬТРА (v5.2.9)
# Автоматическая генерация из geonamescache (15000+ городов)
# + Ручной список редких городов (не в geonames)
# ============================================
...
class GoogleAutocompleteParser:
    ...
    def is_query_allowed(self, query: str, seed: str, country: str) -> bool:
        """
        ПРЕ-ФИЛЬТР (v5.2.9): Проверяет запрос ДО отправки в Google
        ...
        Улучшения v5.2.9:
        - Проверка ЦЕЛЫХ слов (не подстрок!) - исправлен баг "филипс"/"фили"
        - Нормализация для падежей - работает "москве" → "москва"
        - Ручной список редких городов (Щёлкино, Крым)
        - Пересечение множеств для быстрой проверки
        ...
        Примеры:
            ...
            True  # OK: "филипс" - бренд, не город (хоть и содержит "фили")
            
            >>> is_query_allowed("ремонт пылесосов щелкино", "ремонт пылесосов", "ua")
            False  # Блокировка: "щелкино" в ручном списке (Крым)
            
            >>> is_query_allowed("ремонт пылесосов москве", "ремонт пылесосов", "ua")
            False  # Блокировка (норм.): "москве" → "москва"
        """
        ...
        # Извлекаем СЛОВА (не подстроки!) через regex с границами слов
        import re
        ...
        # Используем пересечение множеств (быстро!)
        if blacklist & seed_words:
            return True  # Город в seed - разрешаем (пользователь САМ указал)
        
        # Нормализуем seed слова для проверки падежей
        seed_normalized = self._normalize_words(seed_words) if hasattr(self, 'morph_ru') else seed_words
        
        if blacklist & seed_normalized:
            return True  # Город в seed (нормализованный) - разрешаем
        
        # Проверяем query
        query_lower = query.lower()
        query_words = set(re.findall(r'\b\w+\b', query_lower))
        
        # ЭТАП 1: Проверка исходных слов (для точных совпадений)
        ...
        # ЭТАП 2: Нормализация и проверка (для падежей)
        if hasattr(self, 'morph_ru'):
            query_normalized = self._normalize_words(query_words)
    ...
    def _normalize_words(self, words: set) -> set:
        """
        Нормализация слов через Pymorphy3
        
        Args:
            words: Множество слов
            
        Returns:
            Множество нормализованных слов (именительный падеж, единственное число)
        """
        if not hasattr(self, 'morph_ru'):
            return words

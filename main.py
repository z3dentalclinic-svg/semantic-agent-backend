...
from filters import (
    ...
    apply_l1_5_filter, # ← L1.5 Domain Anchor Filter (между L0 и L2)
...
def apply_filters_traced(result: dict, seed: str, country: str, 
    ...
    if ef == "all":
        ef = "pre,geo,bpf,l0,l15,l2,l3"
    parts = [x.strip() for x in ef.split(",")]
    run_pre = "pre" in parts
    run_geo = "geo" in parts
    run_bpf = "bpf" in parts
    run_l0 = "l0" in parts
    run_l15 = "l15" in parts
    run_l2 = "l2" in parts
    run_l3 = "l3" in parts
    
    logger.info(f"[FILTERS] enabled_filters='{enabled_filters}' → pre={run_pre} geo={run_geo} bpf={run_bpf} l0={run_l0} l1.5={run_l15} l2={run_l2} l3={run_l3}")

...
+ Пре-фильтр - блокирует чужие города ДО отправки в Google
...
app = FastAPI(
    ...
    version="5.2.6",
    description="6 методов | 3 источника | Pre-filter + Entity двухступенчатый | Level 2 Complete"
)
...
class GoogleAutocompleteParser:
    ...
    def is_query_allowed(self, query: str, seed: str, country: str) -> bool:
        """
        ПРЕ-ФИЛЬТР (v5.2.6): Проверяет запрос ДО отправки в Google

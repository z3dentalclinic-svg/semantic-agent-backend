"""
Batch Post-Filter v8.1 - FUNDAMENTAL FIX: GEO DATABASE PRIORITY
Based on Gemini's recommendations for 187 countries support

🔥 ФУНДАМЕНТАЛЬНОЕ ИСПРАВЛЕНИЕ v8.1:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ПРОБЛЕМА v7.7-v7.8:
...
class BatchPostFilter:
    ...
    def _check_geo_conflicts_v75(self, keyword: str, country: str, 
        ...
        for item in search_items:
            ...
            # ✅ v8.1 ФУНДАМЕНТАЛЬНОЕ ИСПРАВЛЕНИЕ: БАЗА ГОРОДОВ = ПЕРВИЧНА
            # 
            ...
            if not found_country:
                ...
                if found_country:
                    logger.debug(f"[v8.1] Found original: '{item}' → {found_country}")
                    item_normalized = item  # Используем оригинал
            elif item_normalized != item:
                logger.debug(f"[v8.1] Found via lemma: '{item}' → '{item_normalized}' → {found_country}")

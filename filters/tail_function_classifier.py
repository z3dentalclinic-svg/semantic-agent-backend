...
class TailFunctionClassifier:
    ...
    def classify(self, tail: str, tail_parses: dict = None, kw: str = "") -> Dict:
        ...
        # ── Fix 2 Diag (блокер 1) ──────────────────────────────────────────
        # Активен при env FIX2_DIAG=1. Для коротких CYR-tail'ов (≤3 слова)
        # логируем: tail, первое слово tail'а есть ли в geo_db и что
        # geo_db про него знает, + id(geo_db) для проверки identity
        # cached-classifier vs fresh geo_db.
        if _FIX2_DIAG:
            _tail_words = tail.lower().split() if tail else []
            if _tail_words and len(_tail_words) <= 3:
                # Только CYR — LATN не интересует для этого диагноза
                _has_cyr = any(
                    '\u0400' <= c <= '\u04ff' for c in tail
                )
                if _has_cyr:
                    _first_word = _tail_words[0]
                    _first_in_db = _first_word in self.geo_db
                    _first_val = (
                        self.geo_db.get(_first_word) if _first_in_db else None
                    )
                    # Биграмма (если есть 2 слова)
                    _bigram = None
                    _bi_in_db = False
                    _bi_val = None
                    if len(_tail_words) >= 2:
                        _bigram = ' '.join(_tail_words[:2])
                        _bi_in_db = _bigram in self.geo_db
                        _bi_val = (
                            self.geo_db.get(_bigram) if _bi_in_db else None
                        )
                    _fix2_logger.warning(
                        "[FIX2_DIAG] kw=%r tail=%r target=%r "
                        "geo_db_id=%d geo_db_len=%d "
                        "first_word=%r in_db=%s val=%s "
                        "bigram=%r in_db=%s val=%s",
                        kw, tail, self.target_country,
                        id(self.geo_db), len(self.geo_db),
                        _first_word, _first_in_db, _first_val,
                        _bigram, _bi_in_db, _bi_val,
                    )

"""
TailFunctionClassifier v22 — исправленный классификатор.
...
class TailFunctionClassifier:
    ...
    def classify(self, tail: str, tail_parses: dict = None, kw: str = "", own_geo: bool = False) -> Dict:
        ...
        # Симметричный блок для СЕРВИСНОГО seed: одиночный перфективный INFN —
        # commitment-интент пользователя ("курсы английского записаться",
        # "доставка цветов оплатить", "ремонт пылесосов забронировать").
        # Морфологическое правило без whitelist'а: INFN + perf aspect.
        # Покрывает записаться/оплатить/забронировать/подписаться/оформить/
        # пройти/начать/купить/заказать — все коммит-глаголы.
        # Impf (бегать, падать, смеяться) — не трактуем как commerce.
        if 'commerce' not in positive_signals and self._seed_is_service:
            _tail_words = tail.lower().split()
            if len(_tail_words) == 1:
                _w = _tail_words[0]
                _p = (tail_parses.get(_w, None) if tail_parses else None)

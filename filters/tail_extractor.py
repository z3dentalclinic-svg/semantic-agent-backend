...
def _all_top_lemmas(word: str) -> frozenset:
    """
    ...
    Кэш: lru_cache(20000) — переживает между батчами и поисками. Уникальных
    слов в типичном датасете ~500-2000. На повторных словах ('цена', 'отзывы',
    'купить') исключает повторный morph.parse и пересоздание set. Возвращает
    frozenset чтобы кэшированные значения были immutable (защита от случайной
    мутации вызывающим кодом).
    """
    ...
    return frozenset(p.normal_form for p in parses if p.score >= threshold)

...
def detect_type_specifier(tail: str, seed: str = "ремонт пылесосов") -> Tuple[bool, str]:
    ...
    "промышленных" + seed "ремонт пылесосов" → род.п. мн.ч. = род.п. мн.ч. → ТИП ✅
    "моющих" → то же → ТИП ✅
    "лучшие" → им.п. ≠ род.п. → НЕ тип ❌
    """
    words = tail.lower().split()
    if not words:
        return False, ""
    
    # Только для коротких хвостов (1-2 слова, чисто прилагательные)
    parsed_first = [morph.parse(w)[0] for w in words]
        ...
        if sp_first.tag.POS == 'NOUN':
            for sp in sp_all:
                if sp.tag.case and sp.tag.number:
                    seed_cases.add((sp.tag.case, sp.tag.number))
            break
    ...
    for w, pf in zip(words, parsed_first):
        ...
        for ap in all_parses:
            if ap.tag.case and ap.tag.number:
                if (ap.tag.case, ap.tag.number) in seed_cases:
                    return True, f"Спецификация типа: '{w}' согласуется с seed"

"""
function_detectors.py — 111 детекторов функции хвоста.
...
def detect_action(tail: str, tp: dict = None) -> Tuple[bool, str]:
    ...
    # 'дома' исключено из substring-паттернов: слишком частая подстрока у
    # NOUN gent от 'дом' ("большого дома", "возле дома", "из дома"). Это
    # приводило к ложным VALID на ключах типа 'хозяйка большого дома'.
    # Проверяется POS-aware ниже: токен 'дома' → action только если
    # единственный pymorphy-разбор с POS=ADVB (наречие), не NOUN.
    ...
    # POS-aware проверка для 'дома' (наречие "at home" vs NOUN gent "дом").
    # 'дома' имеет два разбора в pymorphy:
    #   — ADVB, lemma='дома' (наречие "я буду дома")
    #   — NOUN, inan, masc, sing, gent, lemma='дом' ("возле дома")
    # В контексте детектора action нас интересует только ADVB — это способ
    # выполнения действия. NOUN gent — часть именной группы, не action.
    #
    # Критерий: в tail есть токен 'дома' И первый pymorphy-разбор даёт ADVB.
    # pymorphy возвращает парсы в порядке вероятности по score, поэтому
    # первый разбор = наиболее вероятная POS в изоляции. Для "дома" сам по
    # себе (без контекста) первый парс — NOUN gent (score 0.637 vs ADVB 0.315).
    # Значит одиночный 'дома' как tail → NOUN → не action.
    # Но если перед 'дома' стоят слова типа "заказать", "сделать", "купить"
    # (verb контекст), то морфологически это всё равно 'дома' ADVB.
    # Простейший надёжный критерий: 'дома' — action ТОЛЬКО если оно
    # единственное слово в tail И лемма='дома' (ADVB). Иначе проверка
    # не триггерится, tail идёт дальше по другим детекторам.
    if len(words) == 1 and words[0] == 'дома':
        parsed = _get_parses('дома', tp)[0]
        # Первый разбор pymorphy для 'дома' — NOUN gent. Значит одиночный
        # tail='дома' НЕ action (скорее всего обрывок от "возле дома" или
        # подобного). Эта проверка намеренно консервативна — лучше отправить
        # в GREY и пусть L3 решит, чем дать ложный VALID на "хозяйка дома".
        pass  # явный no-op: не триггерим action для одиночного 'дома'
...
def detect_fragment(tail: str, seed: str = "", tp: dict = None, kw: str = "") -> Tuple[bool, str]:
    ...
    if last_parsed.tag.POS in ('PREP', 'CONJ', 'PRCL') and last_word not in product_suffixes:
        if last_word in misclassified_as_conj:
            ...
            pass
        elif (last_word == 'ин' and len(words) >= 2):
            # Англицизм-идиома "трейд-ин", "чек-ин", "лог-ин", "плаг-ин".
            # pymorphy тегирует 'ин' как PRCL (частица), но в этих биграммах
            # 'ин' — транслитерация английского "in", часть устойчивого термина.
            # Не обрывок, если перед 'ин' стоит слово (любая биграмма).
            # Потенциальный FP: "имя ин" (имя человека "Ин") — крайне редко,
            # гораздо реже чем трейд-ин в e-commerce запросах.
            pass
        else:
...
def detect_broken_grammar(tail: str, tp: dict = None) -> Tuple[bool, str]:
    ...
    if first in prep_cases:
        ...
        #
        # РАСШИРЕННО: даже если POS не None, но pymorphy тегирует слово
        # с case=None для ВСЕХ разборов — это несклоняемое слово (бренд,
        # сокращение, междометие-омоним типа "алло"). pymorphy ошибочно
        # даёт им теги типа PRTS/INTJ/ADVB когда слово не знает в склоняемом
        # виде. Пример:
        #   "авито" → pymorphy: PRTS, case=None (должно быть бренд)
        #   "алло" → pymorphy: INTJ, case=None (бренд магазина)
        #   "rozetka" → POS=None, case=None
        # Если НИ ОДИН парс не даёт реального падежа (case != None) —
        # это несклоняемое слово, не сломанная грамматика.
        second_best = second_parses[0]
        if second_best.tag.POS is None:
            return False, ""
        # Проверка: есть ли у слова хоть один парс с case?
        if not any(p.tag.case for p in second_parses):
            return False, ""

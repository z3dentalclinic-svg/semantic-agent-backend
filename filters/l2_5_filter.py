...
# --- ОТКАТ после теста 3.6 Flash (июль 2026): остаёмся на Flash-Lite ---
# Итог теста: 3.6 обменял 3 инфо-нюанса на 2 региональных/логических среза
# при ×9 по цене; дубли не решил. Регион/дубли лечим промптом V3 и pre_filter.
# MODEL = "gemini-3.6-flash"  # PRICE_IN = 1.50  PRICE_OUT = 7.50  — точка отката
...
L2_5_BUILD = "lite + prompt V3.2 (geo by known absence), 2026-07-27"
...
# «пациентских идиом» (нимесил таблетки, ~6% входа на фарма-сиде) и мигал
# на незнакомых сущностях (танк, sk8).
SYSTEM_PROMPT_V1 = """Проверь список ключевых слов. Дан SEED. Для каждого запроса вынеси вердикт: 1 (валид) или 0 (невалид).
...
# V2 (откат): два критерия + презумпция валидности. Не резал регион и двойное
# гео вовсе; «адвокат сущностей» пропускал сомон-тч / с-европы.
SYSTEM_PROMPT_V2 = """Проверь список ключевых слов. Дан SEED. Для каждого запроса вынеси вердикт: 1 (валид) или 0 (невалид).

Отбрось (0) только то, что:
— не может существовать как запрос, заданный Google живым человеком (битый порядок слов, обрывки, склейка слов, бессмысленный набор токенов);
— содержит сущность, которую ты знаешь и которая не сочетается с SEED ни в каком человеческом поиске.

Название, которого ты не знаешь, считай существующей сущностью из тематики SEED — по нему 0 не ставится.
Фактическая неточность в запросе (несуществующая форма, неверная характеристика) — не повод для 0: люди ищут то, чего не знают.

Всё остальное — валид (1).
Верни только строку из 1 и 0 через запятую, по числу запросов, без пояснений."""

# V3 (откат): полная формулировка. На lite дала коллапс — 51/51 единиц, ноль
# срезов: четыре развёрнутых критерия + гейт «знаешь наверняка» оказались
# тяжелее возможностей модели, все ворота среза закрылись.
SYSTEM_PROMPT_V3 = """Проверь список ключевых слов. Дан SEED. Для каждого запроса вынеси вердикт: 1 (валид) или 0 (невалид).

Отбрось (0) только то, что:
— не может существовать как запрос, заданный Google живым человеком (битый порядок слов, обрывки, склейка слов, бессмысленный набор токенов);
— содержит сущность, которую ты знаешь и которая не сочетается с SEED ни в каком человеческом поиске;
— содержит сущность, которую ты знаешь и которая существует или работает только в другой стране, чем регион поиска;
— содержит место, которое ты знаешь и которое находится в другом населённом пункте или стране, чем локация SEED.

Города, районы, улицы и объекты внутри локации SEED — валид.
Страны и сущности, названные в самом SEED, критериями региона и места не режутся.
Название, которого ты не знаешь, считай существующей сущностью из тематики SEED — по нему 0 не ставится.
Не придумывай запросу оправданий и сценариев, которых в нём нет: решай строго по написанному и по тому, что знаешь наверняка.
Фактическая неточность в запросе (несуществующая форма, неверная характеристика) — не повод для 0: люди ищут то, чего не знают.

Всё остальное — валид (1).
Верни только строку из 1 и 0 через запятую, по числу запросов, без пояснений."""

# V3.1 (откат, не деплоился): сжатая версия V3, забракована формулировками —
# «сущность» вместо языка ключевых фраз, слипшееся гео.
SYSTEM_PROMPT_V3_1 = """Проверь список ключевых слов. Дан SEED. Для каждого запроса вынеси вердикт: 1 (валид) или 0 (невалид).

Ставь 0, если запрос:
— обрывок, склейка или битый порядок слов — такое не ищет живой человек;
— содержит известную тебе сущность, которая не сочетается с SEED ни в каком поиске;
— привязан к известной тебе сущности, работающей только в другой стране, чем регион поиска;
— называет известное тебе место в другом городе или стране, чем локация SEED.

Иначе ставь 1.
Незнакомое название = сущность из тематики SEED, по нему всегда 1.
Места внутри локации SEED и сущности, названные в самом SEED, не режутся.
Фактическая ошибка в запросе (не та форма, не та характеристика) — всё равно 1.
Не выдумывай запросу оправданий.
Верни только строку из 1 и 0 через запятую, по числу запросов, без пояснений."""

# V3.2 (активный, утверждён 2026-07-27): без слова «сущность», гео двумя
# строками: чужая страна — по известности названия; улицы/районы/места —
# по ПОЗИТИВНОМУ знанию отсутствия (знает, что нет в городе поиска → 0;
# не знает → 1). Единственный гео-щит — гео из самого SEED.
SYSTEM_PROMPT = """Проверь список ключевых слов. Дан SEED. Для каждого запроса вынеси вердикт: 1 (валид) или 0 (невалид).

Ставь 0 только если запрос:
— обрывок, склейка или битый порядок слов — живой человек так не ищет;
— по смыслу не может относиться к SEED ни в одном реальном поиске;
— содержит известное тебе название из другой страны, чем регион поиска;
— называет улицу, район или место, про которое ты знаешь, что его нет в городе или стране поиска.

Иначе ставь 1.
Незнакомое слово или название — не повод для 0: считай его частью тематики SEED.
Гео, названное в самом SEED, — не повод для 0.
Ошибка человека в запросе (не та форма, не та характеристика) — не повод для 0.
Не выдумывай запросу оправданий.
Верни только строку из 1 и 0 через запятую, по числу запросов, без пояснений."""
